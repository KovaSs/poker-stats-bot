...
### Fixed
- FloatingOneTap: удалён fallback-редирект (VK требует PKCE, простой redirect не работает). При ошибке виджета показывается сообщение, пользователь может повторить
