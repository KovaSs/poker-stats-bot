...
**Дата генерации:** 3/31/2026, 10:11:09 PM
**Обработано файлов:** 70
**Всего элементов (с учётом папок):** 97
...
- [backend/src/telegram/callbacks.ts](#backend-src-telegram-callbacks_ts)
...
// backend/src/telegram/callbacks.ts

```typescript
import { logger } from "@/config/logger";

import { replyWithAutoDelete } from "./middlewares";
import { sendStats, sendTop } from "./handlers";
...
// Обработчик для топа (top_all, top_last_year, top_2024, ...)
callbacks.set(/^top_(.+)$/, async (ctx, match) => {
  const filter = match[1];
  const chatId = ctx.chat!.id;
  let actualFilter: string | undefined;
  ...
  try {
    ...
    logger.error(`[ERROR] top callback: ${error}`);
    await ctx.answerCbQuery("❌ Ошибка при загрузке топа");
...
export const topHandler = async (ctx: CommandContext) => {
  ...
  const keyboard = {
    inline_keyboard: [
      [
        { text: "🏆 Всё время", callback_data: "top_all" },
        { text: "📆 Последний год", callback_data: "top_last_year" },
      ],
      [
        { text: "2024", callback_data: "top_2024" },
        { text: "2025", callback_data: "top_2025" },
        { text: "2026", callback_data: "top_2026" },
      ],
    ],
  };
...
// data/stats.db (base64, size: 122880, mtime: 2026-03-31T18:45:27.011Z, md5: f0c009f0cfb1d29fd2c0f7be3e2572cc)
